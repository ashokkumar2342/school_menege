                    ...
                    <tbody>
                        @foreach ($re_records as $rs_val)
                            <tr>
                                <td>{{ $rs_val->name_e}}</td> 
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger" select-triger="user_id" onclick="if (confirm('Are you Sure To Remove This Record')){callAjax(this,'{{ route('admin.account.district.assign.delete',Crypt::encrypt($rs_val->id)) }}') } else{console_Log('cancel') }"><i class="fa fa-trash"></i> Remove</button>
                                </td> 
                            </tr> 
                        @endforeach
                    </tbody>
