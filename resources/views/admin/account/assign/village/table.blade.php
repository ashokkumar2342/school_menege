<div class="row"> 
    ...
    <div class="col-lg-4 form-group">
        ...
        <span class="fa fa-asterisk"></span>
        <select name="tehsil" class="form-control select2" id="block_select_box" onchange="callAjax(this,'{{ route('admin.common.tehsil.wise.village') }}','village_select_box')" required>
                ...
                <table id="ajax_data_table" class="table table-bordered table-striped table-hover control-label">
                    ...
                    <tbody>
                        @foreach ($rs_records as $rs_val)
                            <tr>
                                <td>{{ $rs_val->block_name}}</td> 
                                <td>{{ $rs_val->vil_name}}</td> 
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger" select-triger="user_id" onclick="if (confirm('Are you Sure To Remove This Record')){callAjax(this,'{{ route('admin.account.village.assign.delete',Crypt::encrypt($rs_val->id)) }}') } else{console_Log('cancel') }"><i class="fa fa-trash"></i> Remove</button>
                                </td> 
                            </tr> 
                        @endforeach
                    </tbody>
